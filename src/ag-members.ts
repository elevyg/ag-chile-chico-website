// export const agMembers = {
//   "h-de-la-patagonia": { placeId: "ChIJY-YrKz4r7b0RsqE46gSZzIc" },
//   "brisas-del-lago": { placeId: "ChIJaaoRokYr7b0REjgJTQrZ7-M" },
//   "costanera-apart": { placeId: "ChIJ9Qzx86Qr7b0R0dYTCLoQJUQ" },
//   "cabañas-el-engaño": { placeId: "ChIJifhlQmwr7b0R9HSGzXZcklA" },
//   "cabañas-frau-schuster": { placeId: "ChIJrZq5k0gr7b0Rk4ZHq8cqMkk" },
//   patagoniaxpress: { placeId: "ChIJKc8480Yr7b0R_Y1oWU00opk" },
//   "garnika-bike": { placeId: "ChIJFTC580Ar7b0Rru2WdKEOoU8" },
//   "chile-chico-explora": { placeId: "ChIJbVP2IEQr7b0Rqz8LOgUMdf8" },
//   "turismo-tramal": {
//     placeId:
//       "EjFNYW51ZWwgUm9kcsOtZ3VleiA0NDMsIENoaWxlIENoaWNvLCBBeXPDqW4sIENoaWxlIjESLwoUChIJt5eqmUYr7b0R4oYfNWy-yAoQuwMqFAoSCYsigc5FK-29EUFDVm7uin4E",
//   },
// };

export const agMembers = {
  "h-de-la-patagonia": {
    name: "H de la Patagonia",
    placeId: "ChIJY-YrKz4r7b0RsqE46gSZzIc",
  },
  "brisas-del-lago": {
    name: "Brisas del Lago",
    placeId: "ChIJaaoRokYr7b0REjgJTQrZ7-M",
  },
  "costanera-apart": {
    name: "Costanera Apart",
    placeId: "ChIJ9Qzx86Qr7b0R0dYTCLoQJUQ",
  },
  "cabañas-el-engaño": {
    name: "Cabañas El Engaño",
    placeId: "ChIJifhlQmwr7b0R9HSGzXZcklA",
  },
  "cabañas-frau-schuster": {
    name: "Cabañas Frau Schuster",
    placeId: "ChIJrZq5k0gr7b0Rk4ZHq8cqMkk",
  },
  patagoniaxpress: {
    name: "Patagoniaxpress",
    placeId: "ChIJKc8480Yr7b0R_Y1oWU00opk",
  },
  "garnika-bike": {
    name: "Garnika Bike",
    placeId: "ChIJFTC580Ar7b0Rru2WdKEOoU8",
  },
  "chile-chico-explora": {
    name: "Chile Chico Explora",
    placeId: "ChIJbVP2IEQr7b0Rqz8LOgUMdf8",
  },
  "turismo-tramal": {
    name: "Turismo Tramal",
    placeId:
      "EjFNYW51ZWwgUm9kcsOtZ3VleiA0NDMsIENoaWxlIENoaWNvLCBBeXPDqW4sIENoaWxlIjESLwoUChIJt5eqmUYr7b0R4oYfNWy-yAoQuwMqFAoSCYsigc5FK-29EUFDVm7uin4E",
  },
  "kon-aiken": {
    name: "Kon Aiken Turismo",
    placeId: "ChIJ3bqoIUQr7b0RvcpfcX5mq78",
  },
};
